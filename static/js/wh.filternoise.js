/**
 * FilterNoise generates noise and is triggered by MIDI style events.
 * It uses pitch information for filter frequency.
 * 
 * @wapl FilterNoise
...
(function (WX, WH) {
	 ...
	 * @constructor
	 */
	function FilterNoise(id, view, preset) {
	    ...
	    this._noise.to(this._filter).to(this._amp).to(this._output);
	    this._noise.start(0);
	}
	 ...
	 */
 	FilterNoise.prototype = {
		...
		info: {
			...
			author: 'Wouter Hisschemöller',
			type: 'Generator',
			description: 'Noise source with filter'
		},
	...
	WX.PlugIn.extendPrototype(FilterNoise, 'Generator');

	// REQUIRED: registering plug-in into WX ecosystem
	WX.PlugIn.register(FilterNoise);
